...
if __name__ == '__main__':
    ...
    for step, (images, labels) in enumerate(tgt_data_loader_eval):
        for image, label in zip(images, labels):
            ...
            if is_ind_with_src:
                predicted = src_classifier(src_encoder(make_variable(torch.unsqueeze(image, 0)))).squeeze_().cpu()
                y_pred.append(predicted)
                y_true.append(label.squeeze_().cpu())
            elif is_ind_with_tgt:
                predicted = tgt_classifier(tgt_encoder(make_variable(torch.unsqueeze(image, 0)))).squeeze_().cpu()
                y_pred.append(predicted)
                y_true.append(label.squeeze_().cpu())
